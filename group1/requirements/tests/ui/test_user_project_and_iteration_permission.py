...
class TestUserProjectAndIterationPermission(unittest.TestCase):
    ...
    def test_user_project_and_iteration_permission(self):
        ...
        driver.find_element_by_id("id_description").send_keys("This project shows that only a project manager permission will allow a user to make projects and iterations and add users to the project.  Samuel L Jackson has this permission.  George does not.")
        ...
        for i in range(60):
            try:
                if self.is_element_present(By.ID, "id_user_role"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        Select(driver.find_element_by_id("id_user_role")).select_by_visible_text("Developer")
        driver.find_element_by_xpath("//a[contains(@data-user-name, 'george')]").click()
        time.sleep(1)
        ...
        driver.find_element_by_id("id_description").clear()
        driver.find_element_by_id("id_description").send_keys("only Samuel can make iterations")
        ...
        for i in range(60):
            try:
                if self.is_element_present(By.LINK_TEXT, "george"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        driver.find_element_by_link_text("george").click()
        ...
        driver.find_element_by_link_text("Close").click()
        driver.find_element_by_css_selector("i.glyphicon.glyphicon-minus").click()
        for i in range(60):
            try:
