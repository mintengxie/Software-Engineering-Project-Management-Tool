...
class RoleTestCase(TestCase):
    ...
    def __addUserAsClient(self):
        ua = UserAssociation(project=self.__project, user=self.__user,role=user_association.ROLE_CLIENT)
        ua.save()

    def __addUserAsDev(self):
        ua = UserAssociation(project=self.__project, user=self.__user,role=user_association.ROLE_DEVELOPER)
        ua.save()
        
    def __addUserAsOwner(self):
        ua = UserAssociation(project=self.__project, user=self.__user,role=user_association.ROLE_OWNER)
        ua.save()        
        
    def testAddStoryAsClient(self):
        self.__addUserAsClient()
        self.assertEquals(user_manager.canCreateStoryInProject(userID=self.__user.id, projectID=self.__project.id), True)
    ...
    def testAddStoryNoAssoc(self):    
        self.assertEquals(user_manager.canCreateStoryInProject(userID=self.__user.id, projectID=self.__project.id), False)
    ...
    def testIsOwner_NoAssoc(self):
        self.assertEquals(user_manager.isOwner(userID=self.__user.id, projectID=self.__project.id), False)
        
    def testIsOwner_IsOwner(self):
        self.__addUserAsOwner()    
        self.assertEquals(user_manager.isOwner(userID=self.__user.id, projectID=self.__project.id), True)  
